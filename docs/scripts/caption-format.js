if (!window.__captionFormatBound) {
  window.__captionFormatBound = true;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Padrão observado: "Exemplo 1.1 (Título do Exemplo)" -> "Exemplo 1.1 - Título do Exemplo"
    ...
    exTitles.forEach(function(el) {
      try {
        ...
        var em = txt.match(exRe);
        if (em) {
          var prefix = em[1].trim();
          var title = em[2].trim();
