...
Para quantificar a excelência de um produto no dia a dia da indústria, nós pegamos a terminologia exata da ISO e a traduzimos para variáveis matemáticas, criando uma ponte conceitual estruturada:

* **O Objeto (ISO) $\rightarrow$ O Produto Total (QTP):** É o resultado final da equação, o índice global de excelência ($f(Q_i)$).
* **As Características da Qualidade (ISO) $\rightarrow$ As Dimensões Avaliadas ($C_i$):** São as variáveis de desempenho, estética e durabilidade que a engenharia vai efetivamente medir no processo.
* **O Requisito e o Grau (ISO) $\rightarrow$ O Peso de Importância ($A_i$):** É a exigência do cliente (ou da norma) que dita qual característica vale mais na composição da nota final.
* **A Conformidade / Desempenho (ISO) $\rightarrow$ A Nota de Qualidade ($Q_i$):** É o valor numérico extraído da inspeção na fábrica. Se o atendimento ao requisito for alto, o $Q_i$ é alto.

Com essa ponte estabelecida, o próximo passo da engenharia é executar a matemática por trás da norma. No próximo capítulo, aprofundaremos esse cálculo por meio do modelo da **Função Qualidade Total do Produto (QTP)** e do **Índice de Atendimento dos Requisitos (IAR)**.
