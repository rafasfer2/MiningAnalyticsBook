...
::: {.example #exm-niveis-smartphone name="Os Quatro Níveis na Indústria Eletrônica"}
Para visualizar essa evolução, observe o desenvolvimento de um **Smartphone**:
1. **Padrão:** O aparelho sai da linha de montagem sem defeitos físicos, com o processador atingindo a velocidade projetada pela engenharia.
2. **Uso:** O sistema operacional é fluido, a tela é legível sob a luz do sol e a bateria dura um dia inteiro de trabalho intenso.
3. **Custo:** A padronização de componentes internos e a automação reduzem o desperdício fabril, permitindo entregar uma tela de alta resolução por um preço competitivo.
4. **Necessidade Latente:** O software introduz, via Inteligência Artificial, um modo de "Foco" que silencia notificações automaticamente no trabalho, resolvendo um problema que o cliente ainda não tinha percebido conscientemente.
:::
...
::: {.example #exm-fatores-restaurante name="Os Quatro Fatores no Setor de Serviços"}
Em um **Restaurante**, o equilíbrio desses fatores dita a percepção final do cliente:
1. **Padrão:** O prato é servido exatamente como descrito na ficha técnica. O bife chega à mesa com o peso e cozimento exatos do projeto da cozinha.
2. **Uso:** O restaurante oferece um menu executivo servido em 15 minutos para quem tem pouco tempo, sendo altamente funcional para o propósito do cliente.
3. **Custo:** A utilização de ingredientes da estação e o *mise en place* organizado evitam desperdícios, permitindo um preço justo e competitivo.
4. **Necessidade Latente:** Antes de o cliente pedir, o garçom oferece um suporte para bolsas ou uma cortesia ao perceber que o grupo está em uma celebração especial, gerando encantamento.
:::
...
* **Padrão:** O equipamento sai da linha de montagem com o chassi soldado milimetricamente conforme o projeto de engenharia, a capacidade de carga é de exatas 240 toneladas e o motor entrega a potência rigorosamente especificada no manual.
* **Uso:** A cabine possui isolamento acústico, ar-condicionado e ergonomia avançada, garantindo que o operador trabalhe um turno inteiro sem fadiga. A suspensão é projetada para lidar perfeitamente com as irregularidades das vias da mina.
* **Custo:** A fabricante adota a padronização de componentes modulares na montagem, reduzindo o custo fabril. Para a mineradora, o caminhão oferece alta eficiência de combustível (*payload* por litro), reduzindo significativamente o custo operacional da lavra.
* **Necessidade Latente:** O equipamento vem de fábrica com telemetria preditiva embarcada. O sistema avisa automaticamente o PCM (*Planejamento e Controle da Manutenção*) sobre a degradação de um componente hidráulico semanas antes da falha, evitando paradas não programadas e encantando a gestão da frota.
:::
...
* **Atendimento:** O cumprimento rigoroso do prazo de entrega do equipamento na mina (montagem (*site assembly*) no prazo) e a prontidão do time comercial para ajustes no contrato.
