...
**Resposta:** Consulte o [Gabarito da Parte I](appendix/gabarito-parte1.qmd#sec-gabarito-1-1){target="_blank"} para a resolução detalhada.

---

## O Mapeamento Sistêmico: ISO 9001, Qualidade e a Modelagem ARM {#sec-mapeamento-iso-arm}

Para que a transição da filosofia normativa para a engenharia de dados seja executável, precisamos mapear exatamente onde cada conceito abstrato de qualidade "encosta" na norma ISO 9001 e como o modelo ARM o transforma em matemática. O diagrama abaixo ilustra o fluxo dessa operacionalização: a norma dita as regras do sistema, o conceito de qualidade define os atributos vitais e a ARM converte tudo isso em números gerenciáveis.

```{mermaid}
%%| label: fig-fluxo-iso-arm
%%| fig-cap: "Diagrama de Operacionalização: Da ISO 9001 ao Motor Matemático ARM"
flowchart TB
    ...
    Qualidade --> |Operacionaliza em números| ARM_Model
...
Para sistematizar essa integração na rotina da engenharia de produção, a tabela a seguir cruza as oito partes conceituais do modelo com as cláusulas normativas e a sua respectiva tradução para o algoritmo ARM:

| Conceito da Qualidade | Cláusulas ISO 9001 Relacionadas | Tradução Matemática na ARM |
| :--- | :--- | :--- |
...
| **2. Dimensão objetiva e subjetiva** | **9 (Avaliação); 4.2** (Partes interessadas) | Medições reais = **Valores/Notas**; Percepção de importância = **Pesos**. |
...
### O Contexto da Mineração Industrial

Para consolidar essa arquitetura, vejamos como essas premissas regem a operação diária em uma planta de beneficiamento:

1. **Atributo e essência:** A essência do produto é definida por teor metálico, granulometria e umidade. Na ARM, são as *variáveis críticas*.
...
::: {#exr-cap2-britador-problem}
**Seleção de Metodologia: Britador Primário**
Um britador primário apresenta o seguinte panorama: quebra de mandíbula a cada 3 meses; desperdício de 2 horas/semana aguardando peças; faltas de comunicação entre operadores e técnicos de manutenção. Qual(is) abordagem(ns) você recomendaria? Por quê?
:::
