...
flowchart TD
    ...
    %% --- CONFIGURAÇÃO DE ESTILOS (Garantindo letras grandes e legibilidade) ---
    ...
    %% Usando o traço " - " em vez de ponto " . " para evitar o erro de Markdown List
    L --> F1["1 - Comprometimento da Alta Administração"]
    L --> F2["2 - Foco no Consumidor"]
    L --> F8["8 - Benchmarking"]
    ...
    O --> F6["6 - Mensuração da Qualidade"]
    O --> F7["7 - Melhoria Contínua"]
...
7. **Gestão de Processos (Prevenção):** Em vez de apenas olhar o arado pronto no final da linha, a fábrica foca nas condições controladas de cada etapa. A máquina de corte a laser possui sensores que travam a operação se o bico estiver sujo. A qualidade é garantida pela estabilidade do processo.
8. **Melhoria Contínua (Kaizen):** A empresa adota a filosofia de que "hoje deve ser melhor que ontem". Após cada safra, a equipe de engenharia se reúne com os mecânicos de campo para um Ciclo PDCA. Eles analisam pequenos desgastes e decidem mudar o torque de aperto em 5%.
...
1. **Superprodução:** Extrair 10.000 toneladas de ROM (*Run of Mine*) quando o pátio da usina só suporta 5.000. Isso gera um gasto imediato e desnecessário com diesel e operação de maquinário.
2. **Espera:** Uma escavadeira gigantesca parada porque a frota de caminhões demorou a chegar, evidenciando falta de sincronismo logístico.
3. **Transporte:** Mover o estéril (material sem valor comercial) três vezes de lugar antes de levá-lo à pilha definitiva. A movimentação extra de material não agrega valor ao produto final, apenas eleva o custo.
4. **Superprocessamento:** Moer o minério em uma granulometria além da malha exigida pelo cliente. Gastou-se energia elétrica, insumos e tempo de moinho à toa (um caso claro de *Superqualidade*).
5. **Estoque:** Milhões de reais em capital imobilizado na forma de peças de reposição que nunca foram usadas, apenas oxidando nas prateleiras do almoxarifado.
6. **Movimentação:** O mecânico precisar andar 1 km até a oficina toda vez que esquece uma ferramenta específica (foco no movimento inútil do trabalhador).
7. **Defeitos:** Um lote de minério produzido com teor de sílica acima do permitido. Terá que ser misturado (*blendado*) com material rico ou reprocessado na usina, gerando severo retrabalho.
8. **Talento (O Desperdício Intelectual):** O operador da britagem tem uma ideia brilhante para reduzir o desgaste estrutural de uma correia transportadora, mas a gerência tem uma estrutura tão verticalizada que nunca o ouve.
...
%%| fig-cap: "A Casa da Toyota (TPS) - 14 Princípios e Blocos"
flowchart TD
    ...
    %% PILARES E CENTRO: Retângulos e Bordas Arredondadas
    P1["⚙️ PILAR 1: Just-in-Time\nFluxo Contínuo, Puxado, Heijunka"]
    C("👥 CENTRO\nPessoas, Parceiros e\nMelhoria Contínua (Kaizen)")
    P2["🛑 PILAR 2: Jidoka\nParar e Resolver, Poka-Yoke"]
...
%%| fig-cap: "Estrutura Organizacional - Hierarquia de Belts no Seis Sigma"
flowchart BT
    ...
    %% --- ESTRUTURA DOS NÓS (Base para o Topo) ---
    W["⬜ WHITE BELT\nExecutores da Base Operacional"/]
    Y["🟨 YELLOW BELT\nParticipantes Ativos de Projetos"/]
    G["🟩 GREEN BELT\nLíderes Parciais (Conciliam com rotina)"/]
    B["⬛ BLACK BELT\nEspecialistas Dedicados (Full-time)"/]
    M["🟪 MASTER BLACK BELT\nMentores e Gestores do Programa"/]
    C("👑 CHAMPION / SPONSOR\nAlta Liderança e Direcionamento Estratégico")
    ...
    %% Engrossando as linhas de promoção
...
%%| fig-cap: "O Roteiro DMAIC: Metodologia de Solução de Problemas do Seis Sigma"
flowchart LR
...
flowchart TD
    ...
    %% --- ESTRUTURA DOS NÓS (Diferenciando Conceito de Ferramenta) ---
    F{{"🏛️ FUNDAMENTO"}}
    T("📚 TEMAS")
    P["⚙️ PROCESSOS"]
    D["📋 DETALHAMENTO"]
    ...
    %% --- CONEXÕES (Com texto explicativo nas setas) ---
    F == "desdobrado em" ==> T
    T == "concretizados por meio de" ==> P
    P == "explicados por" ==> D
    D == "com sugestão de" ==> M

    %% Engrossando a linha direcional principal
    linkStyle default stroke:#7f8c8d,stroke-width:4px,color:#2c3e50,font-weight:bold
...
1. **Pensamento Sistêmico:** O entendimento do negócio como um sistema integrado. As atividades não podem ser tratadas de forma isolada, exigindo uma visão macro das interdependências internas e externas.
2. **Aprendizado Organizacional e Inovação:** A busca permanente por novos patamares de competência, ciclos contínuos de melhoria e inovação.
3. **Liderança Transformadora:** Liderança ética e inspiradora, atenta a cenários e tendências, capaz de mobilizar as pessoas em torno de valores.
4. **Compromisso com as Partes Interessadas:** Entendimento das necessidades e pactuação de interesses com foco em clientes, unindo a visão de curto e longo prazo à estratégia.
5. **Adaptabilidade:** A busca pelo sucesso através da flexibilidade, agilidade, rapidez nos ciclos de aprendizagem e uso de métodos ágeis.
6. **Desenvolvimento Sustentável:** O compromisso ético e transparente com os impactos sociais e ambientais gerados pela operação.
7. **Orientação por Processos:** Foco em processos bem gerenciados, embasados em dados e informações, visando eficiência e eficácia.
8. **Geração de Valor:** Todos os esforços devem culminar em valor tangível (econômico, social e ambiental) para todas as partes interessadas.
...
flowchart LR
    ...
    %% --- ESTRUTURA DOS NÓS (Formato de "Pílula" para Marcos de Jornada) ---
    A(["❌ Estágio 0\nInexistente"])
    B(["⚠️ Estágio 1\nReativo"])
    C(["🏗️ Estágio 2\nEstruturado"])
    D(["🚀 Estágio 3\nProativo"])
...
![Evolução dos Indicadores na Jornada de Maturidade](image_d7266b.png){#fig-curva-maturidade fig-align="center"}
...
%%| fig-cap: "A Estrutura das 8 Etapas do MASP alinhadas ao Ciclo PDCA"
flowchart LR
...
::: {.callout-note title="📌 Diferença entre PDCA e MASP"}
...
Para resolver o desastre da "Express Mineral" (ou qualquer falha crônica na engenharia de produção), os alunos precisam do MASP. Ele atua como o roteiro analítico e investigativo do PDCA para quando o problema grave já aconteceu e a causa raiz é desconhecida.

O MASP é composto por 8 etapas rigorosas. A @tbl-masp-ferrovia demonstra como aplicá-lo ao caso do Descarrilamento Ferroviário visto no @case-express-mineral:

| Etapa | Ação Estrutural no MASP | Aplicação Prática (Caso Express Mineral) |
| :--- | :--- | :--- |
| **1. Identificação** | Definir o problema e quantificar as perdas. | Descarrilamentos aumentaram 400% no 2º semestre. Prejuízo acumulado de R$ 5 Milhões. |
...
| **5. Execução** | Bloquear as causas ativas. | Treinar maquinistas em direção defensiva e reduzir o peso por vagão para o limite original. |
| **6. Verificação** | Checar se o bloqueio funcionou na prática. | Monitorar a via permanentemente por 3 meses. O índice de quebra de trilhos caiu para zero? |
| **7. Padronização** | Evitar a reincidência sistêmica. | Criar um novo POP de inspeção de via e proibir compras de insumos críticos sem laudo de engenharia. |
...
: Aplicação do MASP em Logística Ferroviária {#tbl-masp-ferrovia}
...
::: {.example #exm-ata-ccq name="Ata de Reunião: Círculo de Controle da Qualidade (CCQ)"}

**Grupo (Nome do Círculo):** _________________________________________________
**Data:** ___/___/______ | **Duração:** (Recomendado 60 min) ____________________
**Líder do Círculo:** _________________________ | **Relator:** _________________________
...
* Causa 1: _________________________________________________________________
* Causa 2: _________________________________________________________________
* Causa 3: _________________________________________________________________
...
* Quem irá fazer: _____________________________________________________________
...
**6. Próxima Reunião:** ___/___/______ às ____:____h
...
%%| fig-cap: "O Ciclo de Vida do Ativo em 7 Etapas (Destaque para a Manutenção)"
flowchart LR
    ...
    %% --- ESTRUTURA DOS NÓS (Formato de Jornada) ---
    S1(["💡 1 - Decisão"])
    S2(["📋 2 - Especificação"])
    S3(["🛒 3 - Aquisição"])
    S4(["⚙️ 4 - Instalação"])
...
%%| fig-cap: "Integração Sistêmica: Do SGQ aos Resultados Operacionais"
flowchart TD
    ...
    %% --- ESTRUTURA DOS NÓS (Com Emojis de Contexto) ---
    A("📘 ISO 9001\nSGQ - Base Normativa e Padronização")
    B("🧠 TQM + Lean + Seis Sigma\nFilosofias e Ferramentas de Excelência")
    C("🏢 Sistemas Corporativos (Ex: VPS / MEG)\nModelo de Gestão Integrada")
    D{{"🏆 Resultados Operacionais\nSegurança, Produção, Qualidade e Custos"}}
...
```{mermaid}
%%| label: fig-mapa-excelencia
%%| fig-cap: "Mapa Mental: O Caminho da Excelência Operacional até o Resultado Sistêmico"
flowchart TD
    ...
    F["<b>🧠 FILOSOFIAS</b><hr>• TQM: Cultura e Pessoas\n• LEAN: Fluxo e Valor (Mudas)\n• 6 SIGMA: Redução de Variação\n• 5S: Base da Estabilidade"]
    M["<b>📚 METODOLOGIAS</b><hr>• ISO 9001: Gestão de Riscos\n• MEG: Maturidade e Aprendizado\n• VPS/BSC: Estratégia e Pilares\n• SGQ: Processos e Requisitos"]
    T["<b>🛠️ FERRAMENTAS</b><hr>• PDCA / MASP: Solução\n• 5S: Base e Organização\n• ISHIKAWA: Causa Raiz\n• 5W2H: Plano de Ação"]
    ...
    ENG["<b>📐 ENGENHARIA</b>\n(Projeto / Padrão)<hr><i>O QUE fazer?</i>"]
    MAN["<b>🔧 MANUTENÇÃO</b>\n(Ativo / Confiabilidade)<hr><i>COMO manter?</i>"]
    OPE["<b>🏭 OPERAÇÃO</b>\n(Uso / Produtividade)<hr><i>QUANTO produzir?</i>"]
