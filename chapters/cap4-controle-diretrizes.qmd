...
### O Paralelo: O Mapa de Juran e o Projeto Informacional
...
No Capítulo 1 desta obra, detalhamos o **Projeto Informacional** e o cálculo da Qualidade Total do Produto (QTP). Aquela estrutura matemática é, na prática, a execução rigorosa e moderna dos passos clássicos de Juran.
...
#| tbl-cap: "Paralelo entre o Mapa de Planejamento de Juran e a Modelagem da Qualidade (IAR e QTP)."
#| echo: false
#| message: false
#| warning: false
...
df_juran <- data.frame(
  ...
  Aplicacao = c(
    "Levantamento da 'Adequação ao Uso' e separação dos Valores (Cliente Externo vs. Fábrica).",
    "Uso da Cadeia de Meios e Fins para converter desejos em 'Elementos Primários' e 'Secundários'.",
    "Cálculo da Função QTP e do IAR, utilizando o Diagrama de Mudge para gerar os pesos ($A_i$).",
    "Integração com a Vertente Técnica (Projeto do Processo) e definição das Metas SMART.",
    "Produção (Conformação/Ajustamento) e início da Zona de Controle da Qualidade operacional."
  )
)

df_juran %>%
  kable(
    col.names = c("Passos", "O Mapa de Juran (Teoria Clássica)", "A Modelagem da Qualidade (Capítulo 1)"),
    align = "cll",
    booktabs = TRUE
  ) %>%
...
Essa correlação evidencia que o "Planejamento" termina quando o Passo 9 é atingido e a equipe de engenharia entrega o **IAR (Índice de Atendimento dos Requisitos)** para o chão de fábrica executar. Neste momento, o processo entra na **Zona de Controle**.

É apenas quando a Qualidade de Conformação (Etapa 3 do Ciclo de Produção) falha e o controle detecta um "Pico Esporádico" que a organização é forçada a acionar a terceira perna da trilogia: a **Melhoria da Qualidade**, operada por meio do ciclo PDCA.
