...
### O Paralelo: Teoria Clássica vs. Engenharia Aplicada

Embora a Trilogia de Juran tenha sido formulada no século XX, seus passos originais para o "Planejamento da Qualidade" continuam sendo o alicerce da engenharia de processos moderna. A grande diferença contemporânea, abordada nos capítulos iniciais desta obra, é a capacidade de quantificar matematicamente etapas que antes eram apenas conceituais.

A @tbl-paralelo-juran demonstra como o roteiro original de 9 passos de Juran se traduz diretamente no modelo de Sistema de Gestão da Qualidade operado nos dias de hoje:

```{r}
#| label: tbl-paralelo-juran
#| tbl-cap: "Paralelo entre o Mapa de Planejamento da Qualidade de Juran e as ferramentas de Engenharia Aplicada."
#| echo: false
#| message: false
#| warning: false
...
df_juran <- data.frame(
  ...
  Juran = c(
    "Identificar os clientes e determinar suas necessidades reais.",
    "Traduzir essas necessidades para a linguagem técnica da empresa.",
  ...
  Aplicacao = c(
    "Levantamento da 'Adequação ao Uso' e das Vertentes da Qualidade (Visão do Cliente vs. Fábrica).",
    "Desenvolvimento do Projeto Informacional e utilização do Diagrama de Mudge para gerar pesos ($A_i$).",
    "Cálculo da Qualidade Total do Produto (QTP) e definição das tolerâncias técnicas.",
    "Integração com o Ciclo de Produção e definição de Metas SMART (Baseline e Target).",
    "Monitoramento através de KPIs, telemetria analítica e início da Zona de Controle de Qualidade."
  )
)

df_juran %>%
  kable(
    col.names = c("Passos Clássicos", "O Mapa de Juran (Teoria Clássica)", "A Engenharia de Qualidade (Prática Atual)"),
    align = "cll",
    booktabs = TRUE
  ) %>%
  ...
  column_spec(3, width = "8cm", border_left = TRUE) %>%
  row_spec(0, background = "#2c3e50", color = "white", bold = TRUE)
```

Essa correlação evidencia que o processo de "Planejamento" não é um fim em si mesmo. Quando o Passo 9 é atingido e a operação assume a rotina, o processo entra na **Zona de Controle**. É apenas quando esse controle detecta uma anomalia (um "Pico Esporádico" ou um desvio crônico) que a organização aciona a terceira perna da trilogia: a **Melhoria da Qualidade**, executada através de metodologias como o PDCA.
